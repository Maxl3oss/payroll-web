...
// Admin
export async function GetSalary(pageNumber: number, pageSize: number, search: string, month: string) {
  try {
    const res = await useAxios.get(`/salary/get-all?search=${search}&pageNumber=${pageNumber}&pageSize=${pageSize}&month=${month}`);
    return res.data;
  } catch (error) {
